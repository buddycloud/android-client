...
public class BuddycloudProvider extends ContentProvider {
	...
	private static class DatabaseHelper extends SQLiteOpenHelper{
		...
		public void onCreate(SQLiteDatabase db) {
			db.execSQL("CREATE TABLE "+TABLE_CHANNELS+" (_id INTEGER PRIMARY KEY,"
					+Channels.NODE_NAME+" VARCHAR,"
			...
			db.execSQL("CREATE TABLE "+TABLE_CHANNEL_DATA+" (_id INTEGER PRIMARY KEY,"
					+ChannelData.ITEM_ID+" VARCHAR,"
					+ChannelData.ITEM_AUTHOR+" VARCHAR,"
			...
			db.execSQL("CREATE TABLE "+TABLE_ROSTER+" (_id INTEGER PRIMARY KEY,"
					+Roster.JID+" VARCHAR,"
					+Roster.NAME+" VARCHAR,"
			...
			db.execSQL("CREATE TABLE "+TABLE_PLACES+" (_id INTEGER PRIMARY KEY,"
					+Places.PLACE_ID+ " VARCHAR,"
					+Places.NAME+" VARCHAR,"
					+Places.LAT+" VARCHAR,"
					+Places.LON+" VARCHAR,"
					+Places.DESCRIPTION+" VARCHAR,"
					+Places.AREA+ " VARCHAR,"
					+Places.COUNTRY + " VARCHAR,"
					+Places.POSTALCODE + " VARCHAR,"
					+Places.REGION + " VARCHAR,"
					+Places.STREET + " VARCHAR,"
					+Places.POPULATION + " VARCHAR,"
					+Places.SHARED + " BOOLEAN,"
					+Places._COUNT +" LONG,"
...
//		context.startService(intent);
		...
		case CHANNELS:
			...
			dummyCursor.addRow(new String[]{"DUMMYCHANNEL","DUMMY TITLE","SOME DESCRIPTION","1"});
			break;
		case CHANNELS_ID:
			qb.setTables(TABLE_CHANNELS);
		...
		case CHANNEL_DATA:
			...
			dummyCursor=new MatrixCursor(BuddyCloud.ChannelData.PROJECTION_MAP);
			
			dummyCursor.addRow(new String[]{
		...
		case ROSTER:
			...
			dummyCursor.addRow(new String[]{"1","dummy@buddycloud.com","DUMMY","Way too long a status comment to be still legible on a single line. Thus we're breaking to as many lines as necessary.","HERE","There","Nowhere"});
			dummyCursor.addRow(new String[]{"2","bebroll@buddycloud.com","Ben","Getting up to speed with Android","Buddycloud Towers","Somewhere with beer","Home"});
			dummyCursor.addRow(new String[]{"3","simon@buddycloud.com","Simon","So many people to cater for!","Buddycloud Towers",null,"Home"});
			break;
		case ROSTER_ID:
			qb.setTables(TABLE_ROSTER);
			qb.appendWhere("_id=" + uri.getPathSegments().get(1));
			...
			dummyCursor=new MatrixCursor(BuddyCloud.Roster.PROJECTION_MAP);
			dummyCursor.addRow(new String[]{"1","dummy@buddycloud.com","DUMMY","STATUS","HERE","There","Nowhere"});
			break;
		case PLACES:
		...
		Cursor c = qb.query(db, projection, selection, selectionArgs, null,
				null, sortOrder);
		//if we have no data in here, we return dummy values
		//TODO: REMOVE FOR PRODUCTION BUILD
		if(c!=null && c.getCount()>0){
			c.setNotificationUri(getContext().getContentResolver(), uri);
			return c;
		}else{
			dummyCursor.setNotificationUri(getContext().getContentResolver(), uri);
			return dummyCursor;
		}
	}
	...
	static{
		...
		CHANNELS_PROJECTION_MAP.put(Channels.CACHE_UPDATE_TIMESTAMP, Channels.CACHE_UPDATE_TIMESTAMP);
		
		CHANNELS_DATA_PROJECTION_MAP.put(ChannelData._ID, ChannelData._ID);
		CHANNELS_DATA_PROJECTION_MAP.put(ChannelData.NODE_NAME, ChannelData.NODE_NAME);
		CHANNELS_DATA_PROJECTION_MAP.put(ChannelData.CONTENT, ChannelData.CONTENT);
		CHANNELS_DATA_PROJECTION_MAP.put(ChannelData.CONTENT_TYPE, ChannelData.CONTENT_TYPE);
		CHANNELS_DATA_PROJECTION_MAP.put(ChannelData.ITEM_AUTHOR, ChannelData.ITEM_AUTHOR);
		CHANNELS_DATA_PROJECTION_MAP.put(ChannelData.ITEM_ID, ChannelData.ITEM_ID);
		CHANNELS_DATA_PROJECTION_MAP.put(ChannelData.GEOLOC_LAT, ChannelData.GEOLOC_LAT);
		CHANNELS_DATA_PROJECTION_MAP.put(ChannelData.GEOLOC_LON, ChannelData.GEOLOC_LON);
		CHANNELS_DATA_PROJECTION_MAP.put(ChannelData.GEOLOC_ACCURACY, ChannelData.GEOLOC_ACCURACY);
		CHANNELS_DATA_PROJECTION_MAP.put(ChannelData.GEOLOC_LOCALITY, ChannelData.GEOLOC_LOCALITY);
		CHANNELS_DATA_PROJECTION_MAP.put(ChannelData.GEOLOC_REGION, ChannelData.GEOLOC_REGION);
		CHANNELS_DATA_PROJECTION_MAP.put(ChannelData.GEOLOC_AREA, ChannelData.GEOLOC_AREA);
		CHANNELS_DATA_PROJECTION_MAP.put(ChannelData.GEOLOC_TEXT, ChannelData.GEOLOC_TEXT);
		CHANNELS_DATA_PROJECTION_MAP.put(ChannelData.GEOLOC_COUNTRY, ChannelData.GEOLOC_COUNTRY);
		CHANNELS_DATA_PROJECTION_MAP.put(ChannelData.GEOLOC_TIMESTAMP, ChannelData.GEOLOC_TIMESTAMP);
		CHANNELS_DATA_PROJECTION_MAP.put(ChannelData.PUBLISHED, ChannelData.PUBLISHED);
		CHANNELS_DATA_PROJECTION_MAP.put(ChannelData.CACHE_UPDATE_TIMESTAMP, ChannelData.CACHE_UPDATE_TIMESTAMP);
		...
		ROSTER_PROJECTION_MAP.put(Roster.CACHE_UPDATE_TIMESTAMP,Roster.CACHE_UPDATE_TIMESTAMP);
		...
		PLACES_PROJECTION_MAP.put(Places.CACHE_UPDATE_TIMESTAMP,Places.CACHE_UPDATE_TIMESTAMP);
