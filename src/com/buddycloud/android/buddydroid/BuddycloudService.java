...
public class BuddycloudService extends Service {
	...
	public void onCreate() {
		...
		String username = pm.getString("jid", "");
		String password = pm.getString("password", "");
		// Create a connection
		mConnection = new BuddycloudClient(username.split("@")[1]);
		try {
			...
			ex.printStackTrace();
			Log
					.e("XMPPClient", "Failed to connect to "
							+ mConnection.getHost());
			Log.e("XMPPClient", ex.toString());
		}

		// retrieve roster
		for (Iterator iterator = mConnection.getRoster().getEntries()
				.iterator(); iterator.hasNext();) {
			RosterEntry buddy = ((RosterEntry) iterator.next());
			Log.d(TAG, buddy.getUser() + " " + buddy.getName() + " "
					+ buddy.getStatus());
		...
		mConnection.getRoster().addRosterListener(new RosterListener() {
			...
			public void presenceChanged(Presence presence) {
				Log.d(TAG, "presence changed: " + presence.getFrom() + " -> "
